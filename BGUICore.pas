...
function  BGUI_CreateGUIcontext :nativeUInt; stdcall;
function  BGUI_DestroyGUIcontext(GUI_Ctx:nativeUInt) :longword; stdcall;
function  BGUI_SetMasterEvent(GUI_Ctx :nativeUInt; TheEvent, TheParm1, TheParm2 :longword) :longword; stdcall;
...
type  BTMiniGUI_Item = object
         id:longword;
         Next :pointer;
         Text:string;
         Data:ansistring;
      end;
      PBTMiniGUI_Item = ^BTMiniGUI_Item;


      BTMiniGUI_Core = class
         private
            LastError          :longint;
            MainThreadHandle   :longword;
            MainThreadID       :longword;
            ...
            ItemsList          :PBTMiniGUI_Item;
         public
            constructor Create(ScrXlng,ScrYlng:longword);
            destructor Destroy; override;
            function SetMasterEvent(TheEvent, TheParm1, TheParm2 :longword) :longword;
//            function SendMessage(TheObj, TheMsg, TheParm1, TheParm2 :longword) :longword;
            function CretaeWindow(drawproc,Executor:pointer; const Text,Data:string; Flags,Style,Parent:longword; Xpos,Ypos:longint; Xlng,Ylng:longword) :longword;
//           Set
//            function SetWindowsText;
//            function GetWindowsText;
      end;
...
//------------------------------------------------------------------------------
function _BMiniGUI_MainThread(a:nativeUInt):longint; stdcall;
...
begin
   ...
   try
      Core := pointer(a);
      while Core.MainThreadRun <> 0 do
      begin

         while Core.Msg_Tail_begin <> Core.Msg_Tail_end do
         begin
            EnterCriticalSection(Core.CritSec);
            with PBGUI_MSGTail(Core.MSG_Tail)[Core.MSG_tail_begin] do
            begin
               aObj := Obj;
               aMsg := Msg;
               aP1  := p1;
            ...
            inc(Core.MSG_tail_begin);
            if Core.MSG_tail_begin > BGUI_MsgTailSize then Core.MSG_tail_begin := 1;
            LeaveCriticalSection(Core.CritSec);
...
constructor BTMiniGUI_Core.Create(ScrXlng,ScrYlng:longword);
begin
   LastError := 0;
   ObjectsList := nil;
   MasterCallBack := nil;
   MasterCallBackParm := 0;
   Msg_Tail_begin := 1;
   Msg_Tail_end := 1;
   Msg_Tail := nil;
   ReallocMem(Msg_Tail,BGUI_MsgTailSize*sizeof(BGUI_Message));
   if BGUI_Core.Msg_Tail = nil then LastError := -1;
   InitializeCriticalSection(CritSec);
   InitializeCriticalSectionandSpinCount(CritSec,2);
   MainThreadRun := 1;
   MainThreadHandle := CreateThread(nil,0,@_BMiniGUI_MainThread,self,0,MainThreadID);
end;

destructor BTMiniGUI_Core.Destroy;
begin
   MainThreadRun := 0; // set signal to terminate
   sleep(500);
   TerminateThread(MainThreadHandle,0);
   CloseHandle(MainThreadHandle);
   DeleteCriticalSection(CritSec);
   if Msg_Tail <> nil then ReallocMem(Msg_Tail,0); // free memory
   inherited;
end;


function   BTMiniGUI_Core.SetMasterEvent(TheEvent, TheParm1, TheParm2 :longword) :longword;
begin

end;

function   BTMiniGUI_Core.CretaeWindow(drawproc,Executor:pointer; const Text,Data:string; Flags,Style,Parent:longword; Xpos,Ypos:longint; Xlng,Ylng:longword) :longword;
var B,O:PBTMiniGUI_Item;
begin
   new(B);
   B.Next := nil;
   B.Text := Text;
...
//------------------------------------------------------------------------------
function BGUI_CreateGUIcontext :nativeUint; stdcall;
var Core:PBTGUI_Core;
begin
   Result := 0; //error
   Core := nil;
   ReallocMem(Core,sizeof(BTGUI_Core));
   if Core <> nil then
   begin
      //initialise context
      Core.Magic := BGUI_CTXmagic;
      Core.ObjectsList := nil;
      Core.MasterCallBack := nil;
      Core.MasterCallBackParm := 0;
      Core.Msg_Tail_begin := 1;
      Core.Msg_Tail_end := 1;
      Core.Msg_Tail := nil;
      ReallocMem(Core.Msg_Tail,BGUI_MsgTailSize*sizeof(BGUI_Message));
      if BGUI_Core.Msg_Tail = nil then
      begin
         ReallocMem(Core,0); //free
         Exit;
      end;

      InitializeCriticalSection(BGUI_Core.CritSec);
      InitializeCriticalSectionandSpinCount(BGUI_Core.CritSec,2);
      Core.MainThreadRun := 1;
      Core.MainThreadFunc := @_BGUI_MainThread;
      Core.MainThreadHandle := CreateThread(nil,0,Core.MainThreadFunc,Core,0,Core.MainThreadID);
      if Core.MainThreadHandle = 0 then
      begin
         ReallocMem(Core.Msg_Tail,0);
         ReallocMem(Core,0);
         Exit;
      end;
   end;
   Result := nativeUInt(Core); //ok
end;
...
begin
   Result := 0;
   try
      Core := pointer(GUI_Ctx);
      if Core.Magic = BGUI_CTXMAGIC then
      begin
         Core.MainThreadRun := 0; // set signal to terminate
         sleep(500);
         TerminateThread(Core.MainThreadHandle,0);
         CloseHandle(Core.MainThreadHandle);
         DeleteCriticalSection(Core.CritSec);
         ReallocMem(Core.Msg_Tail,0); // free memory
         ReallocMem(Core,0); // free memory
         Result := 1;
      end;
   except
      Result := 0;
...
//------------------------------------------------------------------------------
function _BGUI_MainThread(a:nativeUInt):longint; stdcall;
var aObj,aMsg,aP1,aP2:longword;
    Core : PBTGUI_core;
begin
   Result :=0;
