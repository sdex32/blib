...
type
   ...
   _TODBCResults = class
      ...
      public
         ...
         function FieldAsBlob(i :longword): pansichar;
   ...
   BTTinyODBC = class
      ...
      public
         ...
         procedure SetParam(param:longword; blob: pansichar; size: integer); overload;
...
SQLConnect : function (hdbc:longword; szDSN:pansichar; cbDSN:Smallint; szUID:pansichar; cbUID:Smallint; szAuthStr:pansichar; cbAuthStr:Smallint) :Smallint; stdcall;
SQLDisconnect : function (hdbc:longword) :Smallint; stdcall;
SQLExecDirect : function (hstmt:longword;  szSqlStr:pansichar;  cbSqlStr:Integer) :Smallint; stdcall;
SQLPrepare : function (hstmt:longword; szDSN:pansichar; cbDSN:longint) :Smallint; stdcall;
...
begin
   ...
   begin
      ...
      U := AnsiString(user);
      P := AnsiString(pwd);
      res := SQLConnect(ODBCHandle,PAnsiChar(@H[1]),SQL_NTS,PAnsiChar(@U[1]),SQL_NTS,PAnsiChar(@P[1]),SQL_NTS);
      if SQL_SUCCESS <> res then
      begin //error connect
         _ErrorHandle(res);
...
begin
   ...
   begin
      ...
      begin
         if ExecMode <> 0 then
         begin // Execute
            res := SQLExecDirect(aResultSet.Statement, PAnsiChar(@aSQL[1]), Length(aSQL));
...
begin
   ...
   begin
      ...
      if FSqlCode = 0 then
      begin
         res := SQLPrepare(aResultSet.Statement, pansichar(@aSQL[1]), Length(aSQL));
         _ErrorHandle(res);
      end;
   end else _ErrorHandle(-1002);
end;
...
//
procedure    BTTinyODBC.SetParam(param:longword; Blob: PAnsiChar; size: integer);
begin
   SQLBindParameter(aResultSet.Statement,param,SQL_PARAM_INPUT,
...
begin
   ...
   SetLength(data,4096);
   aPapa._ErrorHandle(SQLGetData(Statement,i,SQL_CHAR,pansichar(@data[1]),4096,@returnlength));
...
begin
   ...
   aFirstFetch := 0;
   if i > aCcnt then Exit;
   aPapa._ErrorHandle(SQLGetData(Statement,i,SQL_INTEGER,pansichar(@data),4,@returnlength));
...
begin
   ...
   aFirstFetch := 0;
   if i > aCcnt then Exit;
   aPapa._ErrorHandle(SQLGetData(Statement,i,SQL_DOUBLE,pansichar(@data),sizeof(double),@returnlength));
   if aPapa.SqlCode = 0 then  Result := Data;
end;

//------------------------------------------------------------------------------
function _TODBCResults.FieldAsBlob(i:longword): pansichar;
var Buf:ansistring;
    P,D:pointer;
