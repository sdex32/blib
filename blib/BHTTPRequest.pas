...
{$IFDEF USE_WINHTTP}
      WinHTTPSetTimeouts(pSession,2000,5000,5000,5000);
{$ENDIF}
...
{$IFDEF USE_WINHTTP}
            WinHttpSetOption(pRequest, 31 {WINHTTP_OPTION_SECURITY_FLAGS},@flags,sizeof(flags));
{$ENDIF}
...
{$ENDIF}
                        ...
                        repeat
                           // get response size
{$IFDEF USE_WINHTTP}
                           WinHTTPQueryDataAvailable(pRequest,index);
{$ELSE}
