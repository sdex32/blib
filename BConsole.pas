...
//            aScrXlng   : longword;
         ...
         public
            UserData    :pointer; // for use in runner to send user param
            UserDataS   :ansistring;
            constructor Create;
...
begin
   ...
   begin
      ...
      if c < #32 then continue; //todo tab
      aEnableNL := 1;
      Print(WhereX,WhereY,ss,aTcolor,aBcolor);
      aEnableNL := 0;
      aCurPos := aLastPos;
   end;
end;
...
begin
   ...
   case indx of
      ...
      20: begin
             Result := aHost_Xlng;
             if Result = 0 then Result := 640;
          end;
      21: begin
             Result := aHost_Ylng;
             if Result = 0 then Result := aScrYlng;
          end;
      ...
      60: begin
             Result := 2;
             if aCharHeight = 8   then Result := 0;
             if aCharHeight = 14  then Result := 1;
             if aCharHeight = 16  then Result := 2;
          end;
...
//   if indx = 31  then aExtEvnPar := value;
   ...
   if indx = 60  then begin if value > 2 then value := 2; _SetSystemFont(value); end; // 0-8 1-14 2-16
