...
{ $ DEFINE LOG_HTTPSERVER }

uses Windows,
     BSocket,
     BHTTPRequest,
//     BNetTools,
{$IFDEF LOG_HTTPSERVER}
     BLogFile,
{$ENDIF}
     BFileTools, BStrTools,
     BExecute, BBase64;
...
//         HtmHead :ansistring;
      ...
      BTPageProc = function(var data :BTHTTP_Data; UserParm :NativeUInt) :ansistring; stdcall;
...
var a:BTHTTPServer;
    i,j,k,m,l,Err,TheVerb,prt:longint;
    c:char;
...
{$ENDIF}
   ...
   //      BDebugS('d:\bdebug.log','-2',srv.nSocket);
         i := StrToCase(Verb,[{0}'GET',{1}'POST',{2}'PUT',{3}'PATCH',{4}'DELETE',
                             ...
                             {8}'CONTENT-TYPE',{9}'ACCEPT',{10}'USER-AGENT',{11}'AUTHORIZATION',{12}'SOAPACTION','REFERER']);


         Value := Trim(ParseStr(Line,1,cdel));
         if EL <> 0 then Value := Trim(RightStr(Line,length(Line)-Length(Verb)-2));
   ...
   //      BDebugS('d:\bdebug.log','Parse Vlaue = '+Value,srv.nSocket);
      ...
      begin
         ...
         begin
            inc(k,4);
            if (k+longint(dat.DataLen)-1) <= j then
            begin
...
//Server: TinyWeb/1.94
         ...
         begin
            ...
            repeat
               ...
               if (k>0) then
               begin
                  if Pos(path,dat.URL) = 1  then // start with
                  begin
                     s := ParseStr(a.aConfig.ProxyPass,i,#13);
                     if length(s) > 0 then
                     begin
                        ssl := false;
                        j := 7; // 'http://' 7 chars
                        if Pos('https',s) = 1 then begin ssl := true; j := 8; end;
                        s := RightStr(s,length(s)-j);
                        prt := ToVal(ParseStr(s,1,':'));
                        s := ParseStr(s,0,':');

                        BasePath := '/' + RightStr(dat.URL,length(dat.URL)-k);
                        if length(dat.Que) > 0 then BasePath := BasePath + '?' +dat.Que;

                        j := HTTP_Request(Ansistring(s),prt,ansistring(BasePath),ansistring(dat.SoapAct),'',dat.Data,Response,ssl);
                        if j = 0 then dat.Status := 200;
         ...
         begin
            if (Pos(a.aConfig.procpage, BasePath) = 1)  // starts with
               or ((a.aConfig.Flags and FHTTPServer_Emul)<>0) then // emulation every thing goto proc
            begin
               proc := BTPageProc(a.aConfig.Proc);
               if assigned(proc) then
               begin
                  Response := proc(dat, a.aConfig.ProcParm);  // !IMPORTANT set status on exit proc
               end;
            end;
         end;
...
{$ENDIF}
               ...
               if  (dat.Status = 404) and doindexfile then // .html not found now try .htm   //??? did I need this
               begin
...
//      begin
   ...
   //exceptions
   if UpperCase(dat.Que) = 'WSDL' then fext := 'xml';
   if fext = 'asmx' then fext := 'xml';
